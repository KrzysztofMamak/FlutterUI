...
import 'package:flutter_samples/apps_clone/photo_concept/photo_details_page.dart';
...
const kTopPercentageBefore = 0.7;
const kTopPercentageAfter = 0.55;
const kItemListWidth = 80.0;
...
class _PhotoConceptPageState extends State<PhotoConceptPage> {
  ...
  late ScrollController _scrollControllerMain;
  ...
  Widget _buildIndicator(String title, int count) {
    return Column(
      children: <Widget>[
        Text(
          title.toUpperCase(),
  ...
  void _goToDetails(String image, String tag) async {
    final page = PhotoDetailsPage(
      image: image,
      tag: tag,
    );
  ...
  Widget build(BuildContext context) {
    ...
    const vSeparator = const SizedBox(height: 5.0);
    ...
    if (_expanded) {
      _currentPercentage = kTopPercentageBefore;
      _percentBackground = 1.0;
    } else {
      _currentPercentage = kTopPercentageAfter;
      ...
      child: ListView.builder(
          ...
          itemBuilder: (_, index) {
            ...
            return Padding(
              ...
              child: InkWell(
                onTap: () {
                  _goToDetails(image, tag);
                },
                child: Hero(
                  tag: tag,
                  child: Container(
                    width: kItemListWidth,
                    child: Image.network(
                      image,
                      fit: BoxFit.cover,
                    ),
